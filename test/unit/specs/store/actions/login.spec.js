...
const mockLoginAction = login => {
  // inject-loaderを使ってアクション内の依存関係をモック化するための注入関数を取得
  const actionsInjector = require('@/store/actions');
  ...
  return actionsMocks.deafult.login;
};
...
describe('loginアクション', () => {
  ...
  describe('Auth.loginが成功', () => {
    ...
    it('成功となること', () => {
      ...
      expect(commit.args[0][0].userId).to.equal(userId);
    });
  });
