...
const REGEX_EMAIL = /^(([^<>()[\]\\.,;:\s@"]+(\.[^<>()[\]\\.,;:\s@"]+)*)|(".+"))@((\[[0-9]{1,3}\.[0-9]{1,3}\.[0-9]{1,3}\.[09]{1,3}\])|(([a-zA-Z\-0-9]+\.)+[a-zA-Z]{2,}))$/;
// 二重否定で、全ての値をBooleanに変換する
...
export default {
  ...
  computed: {
    ...
    // validationの各プロパティの真偽値が全て通っているか(trueか)確認
    valid () {
      const validation = this.validation;
      const fields = Object.keys(validation); // [ 'email', 'password' ]
      let valid = true;
      // emailとpasswordの中をさらに分解して確認
    ...
    // validを使ってログイン処理の可否を判定、progressは後述
    disableLoginAction () {
